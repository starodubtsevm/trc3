...
#- 8 Hz channel
		out_buffers[3].append(krl_rec.hz8_fir.proc(out_buffers[2][i]))# signal after 8 Hz filter
		out_buffers[4].append(krl_rec.det8.proc(out_buffers[3][II]))# signal after ask det 8 Hz
		out_buffers[5].append(krl_rec.filt8.proc(out_buffers[4][II]))# signal after flt ask det 8 Hz
		out_buffers[6].append(krl_rec.comp8.proc(out_buffers[5][II]))# signal after comp 8 Hz
#-12 Hz channel
		out_buffers[7].append(krl_rec.hz12_fir.proc(out_buffers[2][i]))# signal after 12 Hz filter
		out_buffers[8].append(krl_rec.det12.proc(out_buffers[7][II]))# signal after ask det 12 Hz
		out_buffers[9].append(krl_rec.filt12.proc(out_buffers[8][II]))# signal after flt ask det 12 Hz
